...
`DealPipelineFacade.recordAndSyncWithPublicDiffs(...)`에 `Collection<DealDiffField>` 입력 API를 추가해 내부에서 기존 `DiffField`로 변환한다.
`QuotationRequestService`는 `dealPipelineFacade.recordAndSyncWithPublicDiffs(...)`를 호출하도록 변경했다.
...
## [2026-03-05] Statement 생성 재시도 트랜잭션 분리 및 결정적 조회

### 변경 대상
- 파일: src/main/java/com/monsoon/seedflowplus/domain/billing/statement/service/StatementService.java
- 파일: src/main/java/com/monsoon/seedflowplus/domain/billing/invoice/repository/InvoiceStatementRepository.java
- 파일: src/main/java/com/monsoon/seedflowplus/domain/deal/log/service/DealPipelineFacade.java

### 변경 내용
`StatementService.createStatement(...)`의 코드 생성/저장 재시도 구간을 `PROPAGATION_REQUIRES_NEW` 트랜잭션 템플릿으로 분리해, 유니크 충돌 시 기존 트랜잭션 rollback-only 전파로 재시도가 무력화되는 문제를 제거했다.
`resolveInvoiceId(...)`는 `findTopByStatementIdAndIncludedTrueOrderByIdDesc(...)`를 사용해 결정적인 invoice 선택 규칙(최신 `invoice_statement_id`)을 적용했다.
`DealPipelineFacade.recordAndSync(...)`는 핵심 입력(`deal`, `docType`, `refId`, `actionType`, `actorType`, `toStage`, `toStatus`)에 대해 fail-fast null 검증을 추가했다.

### 변경 이유
정책 11 — 재시도 로직의 트랜잭션 경계를 분리해 실제 재시도 동작을 보장하고, 비결정적 조회/후행 NPE를 사전에 차단한다.

## [2026-03-11] 계약 승인 일정 동기화 조회 경로 분리
...
- 클래스/메서드: ContractApprovalSchedulesSyncEventHandler.handle, ContractRepository.findByIdWithScheduleRelations

### 변경 내용
계약 승인 일정 동기화 핸들러가 기본 `findById(...)` 대신 deal owner/client를 함께 fetch하는 `findByIdWithScheduleRelations(...)` 조회를 사용하도록 변경했다.
이에 맞춰 `ContractRepository`에 계약, deal, ownerEmp, client를 한 번에 로드하는 전용 조회 메서드를 추가했다.

### 변경 이유
scenario1의 계약 최종 승인 이후 일정 동기화에서 지연 로딩 의존을 줄이고 assignee 계산에 필요한 연관 데이터를 안정적으로 확보하기 위해.
