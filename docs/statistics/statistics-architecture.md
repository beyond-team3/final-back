...
## [2026-03-13] schedule 견적 만료/결제 완료 표시 정책 반영

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/quotation/service/QuotationService.java`
- 클래스/메서드: `QuotationService#createQuotation`, `QuotationService#completeAfterContractApproval`, `QuotationService#deleteQuotation`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/billing/payment/service/PaymentService.java`
- 클래스/메서드: `PaymentService#processPayment`

### 변경 내용
견적 생성 직후 `DealScheduleSyncService`로 만료일 일정을 upsert하고, 견적 완료/삭제 시 같은 external key의 일정을 정리하도록 `QuotationService`를 확장했다.
일정 assignee 해석을 위해 `UserRepository`를 주입하고, 만료 일정 식별용 helper 메서드를 서비스 내부에 추가했다.
`PaymentService`에서는 결제 완료 후 `PAYMENT_RECEIVED` 일정을 생성하던 경로와 관련 의존을 제거해 캘린더에는 결제 마감 일정만 남도록 정리했다.

### 변경 이유
캘린더 정책상 deal 일정에는 견적서 만료일을 노출하고, 결제 완료 일정은 표시하지 않아야 하기 때문이다.
...
## [2026-03-13] 주문 승인 decision 로그 선반영 보정
...
### 변경 내용
ORD 승인 시 after-commit 주문 확정 이벤트는 그대로 발행하되, approval decision 결과값은 실제 주문 상태가 바뀌기 전까지 `PENDING/IN_PROGRESS`를 유지하도록 조정했다.
이제 approval decision 로그가 주문 확정 완료보다 먼저 `CONFIRMED`로 기록되지 않고, 실제 주문 확정 로그는 `OrderService#confirmOrderFromApproval` 성공 시점에만 남는다.
