...
## [2026-03-15] v1 견적 재작성의 원본 deal 계승 보강

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/quotation/dto/request/QuotationCreateRequest.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/quotation/service/QuotationService.java`
- 파일: `src/test/java/com/monsoon/seedflowplus/domain/sales/quotation/service/QuotationServiceTest.java`

### 변경 내용
v1 견적 생성 요청에 `sourceQuotationId`를 추가해, 반려 또는 만료된 원본 견적서를 기반으로 재작성하는 경우를 일반 신규 생성과 구분할 수 있게 했습니다.
`QuotationService.createQuotation(...)` 는 `sourceQuotationId`가 들어오면 원본 견적서의 상태/작성자/거래처를 검증한 뒤, 원본 `deal`과 `revision lineage`를 계승한 새 견적서를 생성합니다.
이로써 RFQ 없는 일반 신규 QUO는 계속 새 deal을 생성하고, 반려 후 재작성 QUO만 같은 영업 문맥의 deal 안에서 이어지게 됩니다.

### 변경 이유
`REMODELING_POLICY.md`의 “상위 문서 없이 QUO 생성 시 새 deal 생성”과 “재작성은 새 문서 생성이지만 같은 영업 문맥 안에서 이어져야 함”을 동시에 만족시키기 위함입니다.
