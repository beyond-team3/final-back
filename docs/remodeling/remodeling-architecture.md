...
## [2026-03-15] v1 계약 재작성의 원본 deal 계승 보강

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/contract/dto/request/ContractCreateRequest.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/contract/service/ContractService.java`
- 파일: `src/test/java/com/monsoon/seedflowplus/domain/sales/contract/service/ContractServiceTest.java`

### 변경 내용
v1 계약 생성 요청에 `sourceContractId`를 추가해, 반려 또는 만료된 원본 계약서를 기반으로 재작성하는 경우를 일반 신규 생성과 구분할 수 있게 했습니다.
`ContractService.createContract(...)` 는 `sourceContractId`가 들어오면 원본 계약서의 상태/작성자/거래처를 검증한 뒤, 원본 `deal`과 `revision lineage`를 계승한 새 계약서를 생성합니다.
이로써 QUO 없는 일반 신규 CNT는 계속 새 deal을 생성하고, 반려 후 재작성 CNT만 같은 영업 문맥의 deal 안에서 이어지게 됩니다.

### 변경 이유
`REMODELING_POLICY.md`의 “상위 문서 없이 CNT 생성 시 새 deal 생성”과 “재작성은 새 문서 생성이지만 같은 영업 문맥 안에서 이어져야 함”을 동시에 만족시키기 위함입니다.
