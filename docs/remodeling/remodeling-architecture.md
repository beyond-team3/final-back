...
## [2026-03-15] 재배 캘린더 공통 집계와 운영 알림 스케줄러 연결

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/product/service/ProductCultivationAlertService.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/product/service/CultivationNotificationCandidate.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/product/service/ProductReadService.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/notification/scheduler/CultivationNotificationScheduler.java`
...
### 변경 내용
캘린더 추천/수확 임박과 재배 알림 후보 계산 규칙을 `ProductCultivationAlertService`로 모아, 담당 거래처 `ClientCrop` 기반 품종 매칭 로직을 한 곳에서 재사용하도록 분리했습니다.
운영 경로에는 `CultivationNotificationScheduler`를 추가해 영업사원별 후보를 매일 계산하고, 파종 프로모션과 수확 피드백 알림을 실제 `NotificationCommandService`로 생성하도록 연결했습니다.
알림 생성은 월 기준 시각을 `1일 09:00`으로 계산하고, 중복 차단 기준을 기존 `createdAt` 당일 비교에서 `scheduledAt` 기반으로 전환해 월 단위 예약 알림과 맞도록 조정했습니다.

### 변경 이유
캘린더 API와 운영 알림이 동일한 품종 매칭/시기 계산 규칙을 공유해야 불일치가 생기지 않고, 월 기준 재배 데이터로도 중복 없이 실제 발송 경로에 연결할 수 있기 때문입니다.

## [2026-03-15] 영업히스토리 목록에서 bootstrap/cancel-only deal 제외

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/deal/core/repository/SalesDealQueryRepositoryImpl.java`
- 파일: `src/test/java/com/monsoon/seedflowplus/domain/deal/core/repository/SalesDealQueryRepositoryTest.java`

### 변경 내용
영업히스토리와 `/api/v2/deals` 가 함께 사용하는 `searchDeals(...)` 에 목록 가시성 조건을 추가해, 문서가 전혀 남지 않아 `RFQ/0/PENDING` placeholder 로 중립화된 bootstrap deal과 삭제된 초기 문서(RFQ/QUO/CNT)만 남은 deal을 기본 목록에서 제외하도록 조정했습니다.
회귀 테스트에는 `dealId=930079` 와 같은 bootstrap placeholder 패턴, 그리고 `DELETED QUO` 만 남은 deal이 목록에서 빠지고 실제 진행 이력이 있는 deal은 유지되는 케이스를 추가했습니다.

### 변경 이유
관리자 승인 전 QUO 취소로 실질 문서가 사라진 deal은 영업 기회로 볼 수 없으므로, 영업히스토리 메인 목록에서는 비노출 처리해야 승인 후 취소나 후속 문서가 존재하는 정상 deal과 정책 충돌 없이 구분할 수 있습니다.

## [2026-03-16] 계약 승인 즉시 활성 전이와 활성 계약 조회 정렬

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/approval/service/ApprovalCommandService.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/deal/log/policy/DocStatusTransitionPolicy.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/contract/repository/ContractRepository.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/sales/contract/service/ContractService.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/billing/invoice/service/InvoiceService.java`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/billing/invoice/scheduler/InvoiceScheduler.java`

### 변경 내용
계약 거래처 승인 시 `ApprovalCommandService`가 승인 시각의 `LocalDate`와 `contract.startDate`를 비교해, 시작일이 승인일과 같거나 이전이면 즉시 `ACTIVE_CONTRACT`, 미래면 `COMPLETED`로 전이하도록 변경했습니다.
이에 맞춰 CNT 승인 전이 정책에 `WAITING_CLIENT -> ACTIVE_CONTRACT`를 추가했고, 거래처 활성 계약 조회는 더 이상 `COMPLETED + 기간내` 우회 조건을 쓰지 않고 `ACTIVE_CONTRACT`만 반환하도록 정리했습니다.
수동/자동 청구서 생성도 활성 계약만 대상으로 좁혀 `COMPLETED`가 미래 시작 승인 대기 의미로 남도록 맞췄고, 상태 동기화 배치의 `COMPLETED -> ACTIVE_CONTRACT` 구간은 미래 시작 계약의 시작일 도래 처리 의미로 유지했습니다.

### 변경 이유
계약 승인 시점에 이미 시작일이 도래한 계약을 배치까지 `COMPLETED`로 남겨두면 deal stage, 활성 계약 조회, 청구 가능 조건이 서로 다른 의미를 보게 되므로, 승인 시점에 상태 의미를 즉시 정렬하기 위함입니다.

## [2026-03-16] 문서 목록 이름 컬럼 조인 보강

### 변경 대상
- 파일: `src/main/resources/db/migration/V1__create_v_document_summary.sql`
- 클래스/메서드: `v_document_summary`
...
- 클래스/메서드: `DocumentSummaryQueryService#toResponse`

### 변경 내용
`v_document_summary` 뷰에 `client_name`, `owner_employee_name` 컬럼을 추가하고 각 문서 타입별로 거래처와 deal owner를 조인하도록 확장했습니다.
읽기 전용 엔티티 `DocumentSummary` 에 신규 컬럼을 매핑하고, 문서 목록 응답 DTO가 해당 값을 그대로 반환하도록 서비스 매핑을 수정했습니다.
이제 `/api/v1/documents` 응답의 `clientName`, `ownerEmployeeName` 이 더 이상 `null` 고정값으로 내려가지 않습니다.

### 변경 이유
문서 목록 화면이 응답의 거래처명과 담당자명을 그대로 렌더링하므로, 조회 모델 단계에서 이름 정보를 함께 제공해야 하기 때문입니다.

## [2026-03-17] 거래처/문서 조회 가시성 필터 정렬

### 변경 대상
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/deal/core/service/SalesDealQueryService.java`
- 클래스/메서드: `SalesDealQueryService#enforceScope`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/deal/core/repository/SalesDealSearchCondition.java`
- 클래스/메서드: `SalesDealSearchCondition`
- 파일: `src/main/java/com/monsoon/seedflowplus/domain/deal/core/repository/SalesDealQueryRepositoryImpl.java`
- 클래스/메서드: `SalesDealQueryRepositoryImpl#searchDeals`
...
### 변경 내용
거래처 사용자의 `/api/v1/deals` 조회에는 `clientPostAdminApprovalOnly` 조건을 추가해, 최신 문서가 QUO/CNT 이면서 단계가 `PENDING_ADMIN` 또는 `REJECTED_ADMIN` 인 deal을 목록에서 제외하도록 했습니다.
문서 목록 조회는 쿼리 레벨에서 `DELETED` 상태를 다시 제외하고, 거래처 역할에서는 QUO/CNT 의 `WAITING_ADMIN`, `REJECTED_ADMIN` 상태를 숨기도록 보강했습니다.
동시에 `v_document_summary` 정의의 RFQ/QUO/CNT 컬럼을 명시적으로 qualify 해 H2 테스트 환경에서도 동일 뷰를 안정적으로 재생성할 수 있게 맞췄습니다.

### 변경 이유
거래처 뷰에서 관리자 승인 전 또는 관리자 반려 문서가 보이면 승인 정책과 사용자 가시성이 충돌하고, 삭제 문서가 남아 보이면 soft-delete 정책과 문서 목록 응답이 어긋나므로 조회 계층에서 역할별 가시성을 명시적으로 강제하기 위함입니다.
