...
| **이경민** | 개발 단계에서는 병해충-품종 매칭 지도, 영업 노트, AI 기반 영업 브리핑 및 전략 분석 기능을 구현하며 실제로 쓰일 수 있는 기능을 만드는 데 집중했습니다. 병해충-품종 매칭 지도에서는 데이터를 얼마나 직관적으로 보여줄 수 있을지 고민했고, 영업 노트에서는 입력 흐름과 조회 편의성을 어떻게 하면 덜 불편하게 만들 수 있을지 계속 수정해 나갔습니다. AI 기능에서는 단순 요약이 아니라 실제 영업에 도움이 되는 결과를 추출해내는 데 초점을 맞췄습니다. 그 과정에서 자사 데이터를 함께 활용하기 위해 RAG를 도입하게 되었고 질문을 임베딩한 뒤 유사도 기반으로 데이터를 찾고 그 결과를 LLM에 전달해 답변을 생성하는 흐름을 구성했습니다. 이 과정을 구조적으로 관리하기 위해 LangChain도 함께 적용했습니다. 초기에는 RAG와 LangChain에 대한 이해가 부족해 구현에 어려움을 겪었지만, 멘토링을 통해 개념부터 다시 정리한 뒤 적용하면서 점점 감을 잡을 수 있었습니다. 단순히 기능을 만드는 것을 넘어서, 새로운 기술을 빠르게 이해하고 실제 서비스에 녹여내는 경험을 할 수 있었던 점이 가장 의미 있었습니다.|
