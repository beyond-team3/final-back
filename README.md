...
### 영업 파이프라인 관리
- **견적 요청-계약-주문**: 견적 요청부터 최종 주문서 생성까지의 워크플로우 자동화
- **실시간 승인 프로세스**: 역할별 권한 제어를 통한 문서 승인/반려 및 실시간 알림 시스템
- **통합 문서 관리**: 표준화된 양식의 PDF 생성 및 이력 추적

### AI 영업 노트
- **맞춤 브리핑**: 영업 노트를 기반으로 AI가 거래처 맞춤형 영업 전략 제안
- **종합 전략 리포트**: LLM(Gemini)을 활용한 데이터 요약 및 핵심 인사이트 추출

### 품종 유사도 시각화
- **품종 유사도 그래프**: 상품 간 관계를 그래프 형태로 표현하여 유사 품종 정보 제공
- **상품 비교 분석**: 여러 상품의 스펙을 한 눈에 비교할 수 있는 대조표 지원

### 병해충-품종 매칭 지도
- **병해충-품종 매칭 지도**: 시기와 지역별 실제 병해충 데이터를 기반으로, 거래처에 적합한 품종 추천. 
- **거래처 스코어링**: 또한 내부 거래처 중요도 점수를 활용해 방문이 시급한 거래처의 위치를 한눈에 파악 가능
...
### 향후 발전 방향
- **판매 상품 확장: 농자재 및 작물보호제**: 현재 종자로 한정짓던 판매 상품을 농자재와 작물보호제로 확장하여 영업의 다양성과 기회를 늘리고자 함
- **전자 결재 시스템 사용**: 서류 기능이 많은 만큼, 전자 결재 시스템을 도입할 예정임. 실제 업무에 바로 적용할 수 있어 사용자에게 도움이 될 것으로 예상 됨
- **모바일 연동 서비스 제공**: 모바일로도 원할한 UI/UX를 제공하여 이동중에도 서비스를 이용할 수 있게할 예정
