...
public class ProductSimilarityService {
    ...
    public SimilarProductResponse getSimilarProducts(Long productId, int limit,
        ...
        // 재배적기 일괄 조회 → Map<productId, CultivationTime>
        List<Long> candidateIds = candidates.stream().map(Product::getId).toList();
        Map<Long, CultivationTime> ctMap = cultivationTimeRepository.findAllByProductIdIn(candidateIds)
                .stream()
        ...
        List<SimilarProductItem> similarProducts = candidates.stream()
                .map(candidate -> {
                    double score = calculateTotalScore(base, baseCt, candidate,
                            ctMap.get(candidate.getId()), activeCriteria);
    ...
    private double calculateTotalScore(Product base, CultivationTime baseCt,
                                       Product candidate, CultivationTime candidateCt,
                                       Set<String> activeCriteria) {
        double tagScore         = calculateTagSimilarity(base.getTags(), candidate.getTags(), activeCriteria);
        double cultivationScore = calculateCultivationSimilarity(baseCt, candidateCt);
    ...
    private double calculateTagSimilarity(Map<String, List<String>> baseTags,
        ...
        for (Map.Entry<String, Double> entry : TAG_CATEGORY_WEIGHTS.entrySet()) {
            ...
            if (!activeCriteria.contains(key)) continue;
            ...
            if (baseSet.isEmpty() && candidateSet.isEmpty()) continue;
    ...
    private double calculateCultivationSimilarity(CultivationTime base, CultivationTime candidate) {
        if (base == null || candidate == null) return 0.0;
        ...
        if (sowing != null) overlaps.add(sowing);
        ...
        if (planting != null) overlaps.add(planting);
        ...
        if (harvesting != null) overlaps.add(harvesting);

        if (overlaps.isEmpty()) return 0.0;
