...
public class BillingRevenueStatisticsQueryService {
    ...
    public List<MonthlyBilledRevenueDto> getMonthlyRevenue(
            BillingRevenueStatisticsFilter filter,
            CustomUserDetails principal
    ) {
        validateFilter(filter);
        return repository.findMonthlyRevenue(filter, resolveScopedEmployeeId(principal));
    }

    public List<CategoryBilledRevenueDto> getCategoryRevenue(
            BillingRevenueStatisticsFilter filter,
            CustomUserDetails principal
    ) {
        validateFilter(filter);
        return repository.findCategoryRevenue(filter, resolveScopedEmployeeId(principal));
    }

    public List<MonthlyCategoryBilledRevenueDto> getMonthlyCategoryRevenue(
            BillingRevenueStatisticsFilter filter,
            CustomUserDetails principal
    ) {
        validateFilter(filter);
        return repository.findMonthlyCategoryRevenue(filter, resolveScopedEmployeeId(principal));
    }
    ...
    private Long resolveScopedEmployeeId(CustomUserDetails principal) {
